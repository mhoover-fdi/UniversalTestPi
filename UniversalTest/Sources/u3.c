...
#include "../headers/LabJackM.h" // PORT
...
{
    long num_long = 0;
// port    struct timeval tv;

 // port   gettimeofday(&tv, NULL);

 // port   return (tv.tv_sec * 1000) + (tv.tv_usec / 1000);
...
int LJTDAC_output(HANDLE hDevice, u3TdacCalibrationInfo *caliInfo, float digital2analog)
{
    uint8 options, speedAdjust, sdaPinNum, sclPinNum;
    ...
    getTdacBinVoltCalibrated(caliInfo, 0, (float)digital2analog, &binaryVoltage);
...
//digital.
int configIO_outputs(HANDLE hDevice)
{
    uint8 sendBuff[12], recBuff[12];
    uint16 checksumTotal;
    int sendChars, recChars;
