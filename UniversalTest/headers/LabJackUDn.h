...
// Device types:
static const long LJ_dtUE9 = 9;
static const long LJ_dtU3 = 3;
static const long LJ_dtU6 = 6;
static const long LJ_dtSMB = 1000;


// Connection types:
static const long LJ_ctUSB = 1; // UE9 + U3 + U6
static const long LJ_ctETHERNET = 2; // UE9 only
static const long LJ_ctETHERNET_MB = 3; // Modbus over Ethernet.  UE9 only.
static const long LJ_ctETHERNET_DATA_ONLY = 4; // Opens data port but not stream port.  UE9 only.
...
static const long LJ_ctUSB_RAW = 101; // UE9 + U3 + U6
static const long LJ_ctETHERNET_RAW = 102; // UE9 only


// IO types:
static const long LJ_ioGET_AIN = 10; // UE9 + U3 + U6.  This is single ended version.
static const long LJ_ioGET_AIN_DIFF = 15; // U3/U6 only.  Put negative channel in x1.  If 32 is passed as x1, Vref will be added to the result.
static const long LJ_ioGET_AIN_ADVANCED = 16; // For testing purposes. 

static const long LJ_ioPUT_AIN_RANGE = 2000; // UE9 + U6
static const long LJ_ioGET_AIN_RANGE = 2001; // UE9 only

// Sets or reads the analog or digital mode of the FIO and EIO pins.  FIO is Channel 0-7, EIO 8-15.
static const long LJ_ioPUT_ANALOG_ENABLE_BIT = 2013; // U3 only
static const long LJ_ioGET_ANALOG_ENABLE_BIT = 2014; // U3 only
...
static const long LJ_ioPUT_ANALOG_ENABLE_PORT = 2015; // U3 only
static const long LJ_ioGET_ANALOG_ENABLE_PORT = 2016; // U3 only

static const long LJ_ioPUT_DAC = 20; // UE9 + U3 + U6
static const long LJ_ioPUT_DAC_ENABLE = 2002; // UE9 + U3 (U3 on Channel 1 only)
static const long LJ_ioGET_DAC_ENABLE = 2003; // UE9 + U3 (U3 on Channel 1 only)

static const long LJ_ioGET_DIGITAL_BIT = 30; // UE9 + U3 + U6.  Changes direction of bit to input as well.
static const long LJ_ioGET_DIGITAL_BIT_DIR = 31; // UE9 + U3 + U6
static const long LJ_ioGET_DIGITAL_BIT_STATE = 32; // UE9 + U3 + U6.  Does not change direction of bit, allowing readback of output.

// Channel is starting bit #, x1 is number of bits to read.
static const long LJ_ioGET_DIGITAL_PORT = 35; // UE9 + U3 + U6  // changes direction of bits to input as well.
static const long LJ_ioGET_DIGITAL_PORT_DIR = 36; // UE9 + U3 + U6
static const long LJ_ioGET_DIGITAL_PORT_STATE = 37; //UE9 + U3 + U6 // does not change direction of bits, allowing readback of output.

// Digital put commands will set the specified digital line(s) to output.
static const long LJ_ioPUT_DIGITAL_BIT = 40; // UE9 + U3 + U6
// Channel is starting bit #, value is output value, x1 is bits to write.
static const long LJ_ioPUT_DIGITAL_PORT = 45; // UE9 + U3 + U6
...
static const long LJ_ioPUT_WAIT = 70; // U3 + U6

// Counter.  Input only.
static const long LJ_ioGET_COUNTER = 50; // UE9 + U3 + U6

static const long LJ_ioPUT_COUNTER_ENABLE = 2008; // UE9 + U3 + U6
static const long LJ_ioGET_COUNTER_ENABLE = 2009; // UE9 + U3 + U6
...
static const long LJ_ioPUT_COUNTER_RESET = 2012;  // UE9 + U3 + U6

// On UE9: timer only used for input. Output Timers don't use these.  Only Channel used.
// On U3/U6: Channel used (0 or 1).
static const long LJ_ioGET_TIMER = 60; // UE9 + U3 + U6

static const long LJ_ioPUT_TIMER_VALUE = 2006; // UE9 + U3 + U6.  Value gets new value.
static const long LJ_ioPUT_TIMER_MODE = 2004; // UE9 + U3 + U6.  On both Value gets new mode.
static const long LJ_ioGET_TIMER_MODE = 2005; // UE9 + U3 + U6
...
static const long LJ_ioSHT_GET_READING = 500; // UE9 + U3 + U6
...
static const long LJ_ioSPI_COMMUNICATION = 503; // UE9 + U3 + U6

static const long LJ_ioI2C_COMMUNICATION = 504; // UE9 + U3 + U6
static const long LJ_ioASYNCH_COMMUNICATION = 505; // UE9 + U3 + U6
static const long LJ_ioTDAC_COMMUNICATION = 506; // UE9 + U3 + U6
...
//
static const long LJ_ioPIN_CONFIGURATION_RESET = 2017; // UE9 + U3 + U6
...
static const long LJ_ioRAW_OUT = 100; // UE9 + U3 + U6
static const long LJ_ioRAW_IN = 101; // UE9 + U3 + U6

static const long LJ_ioRAWMB_OUT = 104; // UE9 Only.  Used with LJ_ctETHERNET_MB to send raw modbus commands to the modbus TCP/IP Socket.
static const long LJ_ioRAWMB_IN = 105;  // UE9 only.  Used with LJ_ctETHERNET_MB to receive raw modbus responses from the modbus TCP/IP Socket.
...
static const long LJ_ioSET_DEFAULTS = 103; // U3 + U6
...
static const long LJ_ioADD_STREAM_CHANNEL = 200; // UE9 + U3 + U6
// Put negative channel in x1.  If 32 is passed as x1, Vref will be added to the result.
static const long LJ_ioADD_STREAM_CHANNEL_DIFF = 206; // U3 + U6

static const long LJ_ioCLEAR_STREAM_CHANNELS = 201; // UE9 + U3 + U6
static const long LJ_ioSTART_STREAM = 202; // UE9 + U3 + U6
static const long LJ_ioSTOP_STREAM = 203; // UE9 + U3 + U6
 
static const long LJ_ioADD_STREAM_DAC = 207; //UE9 only
...
static const long LJ_ioGET_STREAM_DATA = 204; // UE9 + U3 + U6
...
static const long LJ_ioSET_STREAM_CALLBACK = 205; // UE9 + U3 + U6
static const long LJ_ioSET_STREAM_CALLBACK_PTR = 260; // UE9 + U3 + U6. This is for 64-bit compatibility.  The tStreamCallback function is passed as X1 and channel is the User Variable.

// Channel = 0 buzz for a count, Channel = 1 buzz continuous.
// Value is the Period.
// X1 is the toggle count when channel = 0.
static const long LJ_ioBUZZER = 300; // U3 only
...
static const long LJ_ioSET_EVENT_CALLBACK = 400; // UE9 + U3 + U6

// These are the possible EventCodes that will be passed:
static const long LJ_ecDISCONNECT = 1; // Called when the device is unplugged from USB.  No Data is passed.
static const long LJ_ecRECONNECT = 2; // Called when the device is reconnected to USB.  No Data is passed.
static const long LJ_ecSTREAMERROR = 4; // Called when a stream error occurs. Data1 is errorcode, Data2 and Data3 are not used.
// Future events will be power of 2.

// Config iotypes:
static const long LJ_ioPUT_CONFIG = 1000; // UE9 + U3 + U6
static const long LJ_ioGET_CONFIG = 1001; // UE9 + U3 + U6


// Channel numbers used for CONFIG types:
static const long LJ_chLOCALID = 0; // UE9 + U3 + U6
static const long LJ_chHARDWARE_VERSION = 10; // UE9 + U3 + U6 (Read Only)
static const long LJ_chSERIAL_NUMBER = 12; // UE9 + U3 + U6 (Read Only)
static const long LJ_chFIRMWARE_VERSION = 11; // UE9 + U3 + U6 (Read Only)
static const long LJ_chBOOTLOADER_VERSION = 15; // UE9 + U3 + U6 (Read Only)
static const long LJ_chPRODUCTID = 8; // UE9 + U3 + U6 (Read Only)

// UE9 specific:
static const long LJ_chCOMM_POWER_LEVEL = 1; // UE9
static const long LJ_chIP_ADDRESS = 2; // UE9
static const long LJ_chGATEWAY = 3; // UE9
static const long LJ_chSUBNET = 4; // UE9
static const long LJ_chPORTA = 5; // UE9
static const long LJ_chPORTB = 6; // UE9
static const long LJ_chDHCP = 7; // UE9
static const long LJ_chMACADDRESS = 9; // UE9
static const long LJ_chCOMM_FIRMWARE_VERSION = 11; // UE9
static const long LJ_chCONTROL_POWER_LEVEL = 13; // UE9
static const long LJ_chCONTROL_FIRMWARE_VERSION = 14; // UE9 (Read Only)
static const long LJ_chCONTROL_BOOTLOADER_VERSION = 15; // UE9 (Read Only)
static const long LJ_chCONTROL_RESET_SOURCE = 16; // UE9 (Read Only)
static const long LJ_chUE9_PRO = 19; // UE9 (Read Only)

static const long LJ_chLED_STATE = 17; // U3 + U6.  Sets the state of the LED.  Value = LED state.
static const long LJ_chSDA_SCL = 18; // U3 + U6.  Enable/disable SDA/SCL as digital I/O.

static const long LJ_chU3HV = 22; // U3 only (Read Only).  Value will be 1 for a U3-HV and 0 for a U3-LV or a U3 with hardware version < 1.30.

static const long LJ_chU6_PRO = 23; // U6 only.

// Driver related:
// Number of milliseconds that the driver will wait for communication to complete.
static const long LJ_chCOMMUNICATION_TIMEOUT = 20; // For all command-response calls, this is the time after a command that the driver will wait for a response.  USB Default = 1500 ms.  Ethernet Default = 500 ms.
static const long LJ_chSTREAM_COMMUNICATION_TIMEOUT = 21; // This affects the background stream read thread in the driver.  This is the maximum time that can pass between 2 stream data packets from the device.  Default = 500 ms.
...
// buffer of doubles is passed for cal static constants.
...
static const long LJ_chCAL_CONSTANTS = 400; // UE9 + U3 + U6
static const long LJ_chUSER_MEM = 402; // UE9 + U3 + U6
...
static const long LJ_chUSB_STRINGS = 404; // U3 + U6

// Timer/counter related:
static const long LJ_chNUMBER_TIMERS_ENABLED = 1000; // UE9 + U3 + U6
static const long LJ_chTIMER_CLOCK_BASE = 1001; // UE9 + U3 + U6
static const long LJ_chTIMER_CLOCK_DIVISOR = 1002; // UE9 + U3 + U6
static const long LJ_chTIMER_COUNTER_PIN_OFFSET = 1003; // U3 + U6

// AIn related:
static const long LJ_chAIN_RESOLUTION = 2000; // UE9 + U3 + U6
static const long LJ_chAIN_SETTLING_TIME = 2001; // UE9 + U3 + U6
static const long LJ_chAIN_BINARY = 2002; // UE9 + U3 + U6

// DAC related:
static const long LJ_chDAC_BINARY = 3000; // UE9 + U3
...
static const long LJ_chSHT_TEMP = 5000; // UE9 + U3 + U6
static const long LJ_chSHT_RH = 5001; // UE9 + U3 + U6
static const long LJ_chSHT_DATA_CHANNEL = 5002; // UE9 + U3 + U6. Default is FIO0
static const long LJ_chSHT_CLOCK_CHANNEL = 5003; // UE9 + U3 + U6. Default is FIO1

// SPI related:
static const long LJ_chSPI_AUTO_CS = 5100; // UE9 + U3 + U6
static const long LJ_chSPI_DISABLE_DIR_CONFIG = 5101; // UE9 + U3 + U6
static const long LJ_chSPI_MODE = 5102; // UE9 + U3 + U6
static const long LJ_chSPI_CLOCK_FACTOR = 5103; // UE9 + U3 + U6
static const long LJ_chSPI_MOSI_PIN_NUM = 5104; // UE9 + U3 + U6
static const long LJ_chSPI_MISO_PIN_NUM = 5105; // UE9 + U3 + U6
static const long LJ_chSPI_CLK_PIN_NUM = 5106; // UE9 + U3 + U6
static const long LJ_chSPI_CS_PIN_NUM = 5107; // UE9 + U3 + U6

// I2C related:
// Used with LJ_ioPUT_CONFIG
static const long LJ_chI2C_ADDRESS_BYTE = 5108; // UE9 + U3 + U6
static const long LJ_chI2C_SCL_PIN_NUM = 5109; // UE9 + U3 + U6
static const long LJ_chI2C_SDA_PIN_NUM = 5110; // UE9 + U3 + U6
static const long LJ_chI2C_OPTIONS = 5111; // UE9 + U3 + U6
static const long LJ_chI2C_SPEED_ADJUST = 5112; // UE9 + U3 + U6

// Used with LJ_ioI2C_COMMUNICATION:
static const long LJ_chI2C_READ = 5113; // UE9 + U3 + U6
static const long LJ_chI2C_WRITE = 5114; // UE9 + U3 + U6
static const long LJ_chI2C_GET_ACKS = 5115; // UE9 + U3 + U6
static const long LJ_chI2C_WRITE_READ = 5130; // UE9 + U3 + U6

// ASYNCH related:
// Used with LJ_ioASYNCH_COMMUNICATION
static const long LJ_chASYNCH_RX = 5117; // UE9 + U3 + U6
static const long LJ_chASYNCH_TX = 5118; // UE9 + U3 + U6
static const long LJ_chASYNCH_FLUSH = 5128; // UE9 + U3 + U6
static const long LJ_chASYNCH_ENABLE = 5129; // UE9 + U3 + U6

// Used with LJ_ioPUT_CONFIG and LJ_ioGET_CONFIG
static const long LJ_chASYNCH_BAUDFACTOR = 5127; // UE9 + U3 + U6

// LJ TickDAC related:
static const long LJ_chTDAC_SCL_PIN_NUM = 5119; // UE9 + U3 + U6: Used with LJ_ioPUT_CONFIG.
// Used with LJ_ioTDAC_COMMUNICATION
static const long LJ_chTDAC_SERIAL_NUMBER = 5120; // UE9 + U3 + U6: Read only.
static const long LJ_chTDAC_READ_USER_MEM = 5121; // UE9 + U3 + U6
static const long LJ_chTDAC_WRITE_USER_MEM = 5122; // UE9 + U3 + U6
static const long LJ_chTDAC_READ_CONSTANTS = 5123; // UE9 + U3 + U6
static const long LJ_chTDAC_WRITE_CAL_CONSTANTS = 5124; // UE9 + U3 + U6
static const long LJ_chTDAC_UPDATE_DACA = 5125; // UE9 + U3 + U6
static const long LJ_chTDAC_UPDATE_DACB = 5126; // UE9 + U3 + U6

// Stream related.  Note: Putting to any of these values will stop any running streams.
static const long LJ_chSTREAM_SCAN_FREQUENCY = 4000; // UE9 + U3 + U6
static const long LJ_chSTREAM_BUFFER_SIZE = 4001;
static const long LJ_chSTREAM_CLOCK_OUTPUT = 4002; // UE9 only
static const long LJ_chSTREAM_EXTERNAL_TRIGGER = 4003; // UE9 only
static const long LJ_chSTREAM_WAIT_MODE = 4004;
static const long LJ_chSTREAM_DISABLE_AUTORECOVERY = 4005; // U3 + U6
static const long LJ_chSTREAM_SAMPLES_PER_PACKET = 4108; // UE9 + U3 + U6. Read only for UE9.
static const long LJ_chSTREAM_READS_PER_SECOND = 4109;
static const long LJ_chAIN_STREAM_SETTLING_TIME = 4110; // U6 only

// Read only stream related
static const long LJ_chSTREAM_BACKLOG_COMM = 4105; // UE9 + U3 + U6
static const long LJ_chSTREAM_BACKLOG_CONTROL = 4106; // UE9 only
static const long LJ_chSTREAM_BACKLOG_UD = 4107;

// Special channel numbers
static const long LJ_chALL_CHANNELS = -1;
static const long LJ_INVALID_CONSTANT = -999;


//Thermocouple Type constants.
static const long LJ_ttB = 6001;
static const long LJ_ttE = 6002;
static const long LJ_ttJ = 6003;
static const long LJ_ttK = 6004;
static const long LJ_ttN = 6005;
static const long LJ_ttR = 6006;
static const long LJ_ttS = 6007;
static const long LJ_ttT = 6008;


// Other constants:

// Ranges (not all are supported by all devices):
static const long LJ_rgBIP20V = 1;   // -20V to +20V
static const long LJ_rgBIP10V = 2;   // -10V to +10V
static const long LJ_rgBIP5V = 3;    // -5V to +5V
static const long LJ_rgBIP4V = 4;    // -4V to +4V
static const long LJ_rgBIP2P5V = 5;  // -2.5V to +2.5V
static const long LJ_rgBIP2V = 6;    // -2V to +2V
static const long LJ_rgBIP1P25V = 7; // -1.25V to +1.25V
static const long LJ_rgBIP1V = 8;    // -1V to +1V
static const long LJ_rgBIPP625V = 9; // -0.625V to +0.625V
static const long LJ_rgBIPP1V = 10;  // -0.1V to +0.1V
static const long LJ_rgBIPP01V = 11; // -0.01V to +0.01V

static const long LJ_rgUNI20V = 101;    // 0V to +20V
static const long LJ_rgUNI10V = 102;    // 0V to +10V
static const long LJ_rgUNI5V = 103;     // 0V to +5V
static const long LJ_rgUNI4V = 104;     // 0V to +4V
static const long LJ_rgUNI2P5V = 105;   // 0V to +2.5V
static const long LJ_rgUNI2V = 106;     // 0V to +2V
static const long LJ_rgUNI1P25V = 107;  // 0V to +1.25V
static const long LJ_rgUNI1V = 108;     // 0V to +1V
static const long LJ_rgUNIP625V = 109;  // 0V to +0.625V
static const long LJ_rgUNIP5V = 110;    // 0V to +0.500V
static const long LJ_rgUNIP25V = 112;   // 0V to +0.25V
static const long LJ_rgUNIP3125V = 111; // 0V to +0.3125V
static const long LJ_rgUNIP025V = 113;  // 0V to +0.025V
static const long LJ_rgUNIP0025V = 114; // 0V to +0.0025V


// Timer modes:
static const long LJ_tmPWM16 = 0; // 16 bit PWM
static const long LJ_tmPWM8 = 1; // 8 bit PWM
static const long LJ_tmRISINGEDGES32 = 2; // 32-bit rising to rising edge measurement
static const long LJ_tmFALLINGEDGES32 = 3; // 32-bit falling to falling edge measurement
static const long LJ_tmDUTYCYCLE = 4; // Duty cycle measurement
static const long LJ_tmFIRMCOUNTER = 5; // Firmware based rising edge counter
static const long LJ_tmFIRMCOUNTERDEBOUNCE = 6; // Firmware counter with debounce
static const long LJ_tmFREQOUT = 7; // Frequency output
static const long LJ_tmQUAD = 8; // Quadrature
static const long LJ_tmTIMERSTOP = 9; // Stops another timer after n pulses
static const long LJ_tmSYSTIMERLOW = 10; // Read lower 32-bits of system timer
static const long LJ_tmSYSTIMERHIGH = 11; // Read upper 32-bits of system timer
static const long LJ_tmRISINGEDGES16 = 12; // 16-bit rising to rising edge measurement
static const long LJ_tmFALLINGEDGES16 = 13; // 16-bit falling to falling edge measurement
static const long LJ_tmLINETOLINE = 14; // Line to Line measurement


// Timer clocks:
static const long LJ_tc750KHZ = 0; // UE9: 750 kHz 
static const long LJ_tcSYS = 1;    // UE9 + U3 + U6: System clock

static const long LJ_tc2MHZ = 10;       // U3: Hardware Version 1.20 or lower
static const long LJ_tc6MHZ = 11;       // U3: Hardware Version 1.20 or lower
static const long LJ_tc24MHZ = 12;      // U3: Hardware Version 1.20 or lower
static const long LJ_tc500KHZ_DIV = 13; // U3: Hardware Version 1.20 or lower
static const long LJ_tc2MHZ_DIV = 14;   // U3: Hardware Version 1.20 or lower
static const long LJ_tc6MHZ_DIV = 15;   // U3: Hardware Version 1.20 or lower
static const long LJ_tc24MHZ_DIV = 16;  // U3: Hardware Version 1.20 or lower

static const long LJ_tc4MHZ = 20;      // U3: Hardware Version 1.21 or higher, + U6
static const long LJ_tc12MHZ = 21;     // U3: Hardware Version 1.21 or higher, + U6
static const long LJ_tc48MHZ = 22;     // U3: Hardware Version 1.21 or higher, + U6
static const long LJ_tc1MHZ_DIV = 23;  // U3: Hardware Version 1.21 or higher, + U6
static const long LJ_tc4MHZ_DIV = 24;  // U3: Hardware Version 1.21 or higher, + U6
static const long LJ_tc12MHZ_DIV = 25; // U3: Hardware Version 1.21 or higher, + U6
static const long LJ_tc48MHZ_DIV = 26; // U3: Hardware Version 1.21 or higher, + U6


// Stream wait modes:
static const long LJ_swNONE = 1; // No wait, return whatever is available.
static const long LJ_swALL_OR_NONE = 2; // No wait, but if all points requested aren't available, return none.
static const long LJ_swPUMP = 11; // Wait and pump the message pump.  Preferred when called from primary thread (if you don't know
...
static const long LJ_swSLEEP = 12; // Wait by sleeping (don't do this in the primary thread of your app, or it will temporarily 
...
static const long LJ_ioSWDT_CONFIG = 507; // UE9 + U3 + U6: Use with LJ_chSWDT_ENABLE or LJ_chSWDT_DISABLE.
static const long LJ_ioSWDT_STROKE = 508; // UE9 only: Used when SWDT_STRICT_ENABLE is turned on to renew the watchdog. 

static const long LJ_chSWDT_ENABLE = 5200; // UE9 + U3 + U6: Used with LJ_ioSWDT_CONFIG to enable watchdog.  Value paramter is number of seconds to trigger.
static const long LJ_chSWDT_DISABLE = 5201; // UE9 + U3 + U6: Used with LJ_ioSWDT_CONFIG to disable watchdog.

// Used with LJ_io_PUT_CONFIG
static const long LJ_chSWDT_RESET_DEVICE = 5202; // U3 + U6: Reset U3 on watchdog reset.  Write only.
static const long LJ_chSWDT_RESET_COMM = 5203; // UE9 only: Reset Comm on watchdog reset.  Write only.
static const long LJ_chSWDT_RESET_CONTROL = 5204; // UE9 only: Reset Control on watchdog trigger.  Write only.
static const long LJ_chSWDT_UPDATE_DIOA = 5205; // UE9 + U3 + U6: Update DIO0 settings after reset.  Write only.
static const long LJ_chSWDT_UPDATE_DIOB = 5206; // UE9 only: Update DIO1 settings after reset.  Write only.
static const long LJ_chSWDT_DIOA_CHANNEL = 5207; // UE9 + U3 + U6: DIO0 channel to be set after reset.  Write only.
static const long LJ_chSWDT_DIOA_STATE = 5208; // UE9 + U3 + U6: DIO0 state to be set after reset.  Write only.
static const long LJ_chSWDT_DIOB_CHANNEL = 5209; // UE9: DIO1 channel to be set after reset.  Write only.
static const long LJ_chSWDT_DIOB_STATE = 5210; // UE9: DIO1 state to be set after reset.  Write only.
static const long LJ_chSWDT_UPDATE_DAC0 = 5211; // UE9 only: Update DAC0 settings after reset.  Write only.
static const long LJ_chSWDT_UPDATE_DAC1 = 5212; // UE9 only: Update DAC1 settings after reset.  Write only.
static const long LJ_chSWDT_DAC0 = 5213; // UE9 only: Voltage to set DAC0 at on watchdog reset.  Write only.
static const long LJ_chSWDT_DAC1 = 5214; // UE9 only: Voltage to set DAC1 at on watchdog reset.  Write only.
static const long LJ_chSWDT_DAC_ENABLE = 5215; // UE9 only: Enable DACs on watchdog reset.  Default is true.  Both DACs are enabled or disabled togeather.  Write only.
static const long LJ_chSWDT_STRICT_ENABLE = 5216; // UE9 only: Watchdog will only renew with LJ_ioSWDT_STROKE command.
static const long LJ_chSWDT_INITIAL_ROLL_TIME = 5217; // UE9 only: Watchdog timer for the first cycle when powered on, after watchdog triggers a reset the normal value is used.  Set to 0 to disable.

// END BETA CONSTANTS


// Error codes:  These will always be in the range of -1000 to 3999 for labView compatibility (+6000).
static const LJ_ERROR LJE_NOERROR = 0;
static const LJ_ERROR LJE_COMMAND_LIST_ERROR = 1; 
static const LJ_ERROR LJE_INVALID_CHANNEL_NUMBER = 2; // Cccurs when a channel that doesn't exist is specified (i.e. DAC #2 on a UE9), or data from streaming is requested on a channel that isn't streaming.
static const LJ_ERROR LJE_INVALID_RAW_INOUT_PARAMETER = 3;
static const LJ_ERROR LJE_UNABLE_TO_START_STREAM = 4;
static const LJ_ERROR LJE_UNABLE_TO_STOP_STREAM = 5;
static const LJ_ERROR LJE_NOTHING_TO_STREAM = 6;
static const LJ_ERROR LJE_UNABLE_TO_CONFIG_STREAM = 7;
static const LJ_ERROR LJE_BUFFER_OVERRUN = 8; // Cccurs when stream buffer overruns (this is the driver buffer not the hardware buffer).  Stream is stopped.
static const LJ_ERROR LJE_STREAM_NOT_RUNNING = 9;
static const LJ_ERROR LJE_INVALID_PARAMETER = 10;
static const LJ_ERROR LJE_INVALID_STREAM_FREQUENCY = 11;
static const LJ_ERROR LJE_INVALID_AIN_RANGE = 12;
static const LJ_ERROR LJE_STREAM_CHECKSUM_ERROR = 13; // Occurs when a stream packet fails checksum.  Stream is stopped.
static const LJ_ERROR LJE_STREAM_COMMAND_ERROR = 14; // Occurs when a stream packet has invalid command values.  Stream is stopped.
static const LJ_ERROR LJE_STREAM_ORDER_ERROR = 15; // Occurs when a stream packet is received out of order (typically one is missing).  Stream is stopped.
static const LJ_ERROR LJE_AD_PIN_CONFIGURATION_ERROR = 16; // Occurs when an analog or digital request was made on a pin that isn't configured for that type of request.
static const LJ_ERROR LJE_REQUEST_NOT_PROCESSED = 17; // When a LJE_AD_PIN_CONFIGURATION_ERROR occurs, all other IO requests after the request that caused the error won't be processed. Those requests will return this error.


// U3 & U6 Specific Errors
static const LJ_ERROR LJE_SCRATCH_ERROR = 19;
static const LJ_ERROR LJE_DATA_BUFFER_OVERFLOW = 20;
static const LJ_ERROR LJE_ADC0_BUFFER_OVERFLOW = 21;
static const LJ_ERROR LJE_FUNCTION_INVALID = 22;
static const LJ_ERROR LJE_SWDT_TIME_INVALID = 23;
static const LJ_ERROR LJE_FLASH_ERROR = 24;
static const LJ_ERROR LJE_STREAM_IS_ACTIVE = 25;
static const LJ_ERROR LJE_STREAM_TABLE_INVALID = 26;
static const LJ_ERROR LJE_STREAM_CONFIG_INVALID = 27;
static const LJ_ERROR LJE_STREAM_BAD_TRIGGER_SOURCE = 28;
static const LJ_ERROR LJE_STREAM_INVALID_TRIGGER = 30;
static const LJ_ERROR LJE_STREAM_ADC0_BUFFER_OVERFLOW = 31;
static const LJ_ERROR LJE_STREAM_SAMPLE_NUM_INVALID = 33;
static const LJ_ERROR LJE_STREAM_BIPOLAR_GAIN_INVALID = 34;
static const LJ_ERROR LJE_STREAM_SCAN_RATE_INVALID = 35;
static const LJ_ERROR LJE_TIMER_INVALID_MODE = 36;
static const LJ_ERROR LJE_TIMER_QUADRATURE_AB_ERROR = 37;
static const LJ_ERROR LJE_TIMER_QUAD_PULSE_SEQUENCE = 38;
static const LJ_ERROR LJE_TIMER_BAD_CLOCK_SOURCE = 39;
static const LJ_ERROR LJE_TIMER_STREAM_ACTIVE = 40;
static const LJ_ERROR LJE_TIMER_PWMSTOP_MODULE_ERROR = 41;
static const LJ_ERROR LJE_TIMER_SEQUENCE_ERROR = 42;
static const LJ_ERROR LJE_TIMER_SHARING_ERROR = 43;
static const LJ_ERROR LJE_TIMER_LINE_SEQUENCE_ERROR = 44;
static const LJ_ERROR LJE_EXT_OSC_NOT_STABLE = 45;
static const LJ_ERROR LJE_INVALID_POWER_SETTING = 46;
static const LJ_ERROR LJE_PLL_NOT_LOCKED = 47;
static const LJ_ERROR LJE_INVALID_PIN = 48;
static const LJ_ERROR LJE_IOTYPE_SYNCH_ERROR = 49;
static const LJ_ERROR LJE_INVALID_OFFSET = 50;
static const LJ_ERROR LJE_FEEDBACK_IOTYPE_NOT_VALID = 51;
static const LJ_ERROR LJE_CANT_CONFIGURE_PIN_FOR_ANALOG = 67;
static const LJ_ERROR LJE_CANT_CONFIGURE_PIN_FOR_DIGITAL = 68;
static const LJ_ERROR LJE_TC_PIN_OFFSET_MUST_BE_4_TO_8 = 70;
static const LJ_ERROR LJE_INVALID_DIFFERENTIAL_CHANNEL = 71;
static const LJ_ERROR LJE_DSP_SIGNAL_OUT_OF_RANGE = 72;

// Other errors
static const LJ_ERROR LJE_SHT_CRC = 52;
static const LJ_ERROR LJE_SHT_MEASREADY = 53;
static const LJ_ERROR LJE_SHT_ACK = 54;
static const LJ_ERROR LJE_SHT_SERIAL_RESET = 55;
static const LJ_ERROR LJE_SHT_COMMUNICATION = 56;

static const LJ_ERROR LJE_AIN_WHILE_STREAMING = 57;

static const LJ_ERROR LJE_STREAM_TIMEOUT = 58;
static const LJ_ERROR LJE_STREAM_CONTROL_BUFFER_OVERFLOW = 59;
static const LJ_ERROR LJE_STREAM_SCAN_OVERLAP = 60;
static const LJ_ERROR LJE_FIRMWARE_VERSION_IOTYPE = 61;
static const LJ_ERROR LJE_FIRMWARE_VERSION_CHANNEL = 62;
static const LJ_ERROR LJE_FIRMWARE_VERSION_VALUE = 63;
static const LJ_ERROR LJE_HARDWARE_VERSION_IOTYPE = 64;
static const LJ_ERROR LJE_HARDWARE_VERSION_CHANNEL = 65;
static const LJ_ERROR LJE_HARDWARE_VERSION_VALUE = 66;

static const LJ_ERROR LJE_LJTDAC_ACK_ERROR = 69;

static const LJ_ERROR LJE_STREAM_INVALID_CONNECTION = 73;

static const LJ_ERROR LJE_MIN_GROUP_ERROR = 1000; // All errors above this number will stop all requests, below this number are request level errors.

static const LJ_ERROR LJE_UNKNOWN_ERROR = 1001; // Occurs when an unknown error occurs that is caught, but still unknown.
static const LJ_ERROR LJE_INVALID_DEVICE_TYPE = 1002; // Occurs when devicetype is not a valid device type.
static const LJ_ERROR LJE_INVALID_HANDLE = 1003; // Occurs when invalid handle used.
static const LJ_ERROR LJE_DEVICE_NOT_OPEN = 1004;  // Occurs when Open() fails and AppendRead called despite.
static const LJ_ERROR LJE_NO_DATA_AVAILABLE = 1005; // This is cause when GetResult() called without calling Go/GoOne(), or when GetResult() passed a channel that wasn't read.
static const LJ_ERROR LJE_NO_MORE_DATA_AVAILABLE = 1006;
static const LJ_ERROR LJE_LABJACK_NOT_FOUND = 1007; // Occurs when the LabJack is not found at the given id or address.
static const LJ_ERROR LJE_COMM_FAILURE = 1008; // Occurs when unable to send or receive the correct number of bytes.
static const LJ_ERROR LJE_CHECKSUM_ERROR = 1009;
static const LJ_ERROR LJE_DEVICE_ALREADY_OPEN = 1010; // Occurs when LabJack is already open via USB in another program or process.
static const LJ_ERROR LJE_COMM_TIMEOUT = 1011;
static const LJ_ERROR LJE_USB_DRIVER_NOT_FOUND = 1012;
static const LJ_ERROR LJE_INVALID_CONNECTION_TYPE = 1013;
static const LJ_ERROR LJE_INVALID_MODE = 1014;
static const LJ_ERROR LJE_DEVICE_NOT_CONNECTED = 1015; // Occurs when a LabJack that was opened is no longer connected to the system.
...
static const LJ_ERROR LJE_DISCONNECT = 2000; 
static const LJ_ERROR LJE_RECONNECT = 2001;

// and an area for your own codes.  This area won't ever be used for LabJack codes.
static const LJ_ERROR LJE_MIN_USER_ERROR = 3000;
static const LJ_ERROR LJE_MAX_USER_ERROR = 3999;


// Warning are negative
static const LJ_ERROR LJE_DEVICE_NOT_CALIBRATED = -1; // Defaults used instead
static const LJ_ERROR LJE_UNABLE_TO_READ_CALDATA = -2; // Defaults used instead
...
static const long LJ_ioANALOG_INPUT = 10;
static const long LJ_ioANALOG_OUTPUT = 20; // UE9 + U3
static const long LJ_ioDIGITAL_BIT_IN = 30; // UE9 + U3
static const long LJ_ioDIGITAL_PORT_IN = 35; // UE9 + U3
static const long LJ_ioDIGITAL_BIT_OUT = 40; // UE9 + U3
static const long LJ_ioDIGITAL_PORT_OUT = 45; // UE9 + U3
static const long LJ_ioCOUNTER = 50; // UE9 + U3
static const long LJ_ioTIMER = 60; // UE9 + U3
static const long LJ_ioPUT_COUNTER_MODE = 2010; // UE9
static const long LJ_ioGET_COUNTER_MODE = 2011; // UE9
static const long LJ_ioGET_TIMER_VALUE = 2007; // UE9
static const long LJ_ioCYCLE_PORT = 102;  // UE9 
static const long LJ_chTIMER_CLOCK_CONFIG = 1001; // UE9 + U3
static const long LJ_ioPUT_CAL_CONSTANTS = 400;
static const long LJ_ioGET_CAL_CONSTANTS = 401;
static const long LJ_ioPUT_USER_MEM = 402;
static const long LJ_ioGET_USER_MEM = 403;
static const long LJ_ioPUT_USB_STRINGS = 404;
static const long LJ_ioGET_USB_STRINGS = 405;
static const long LJ_ioSHT_DATA_CHANNEL = 501; // UE9 + U3
static const long LJ_ioSHT_CLOCK_CHANNEL = 502; // UE9 + U3
static const long LJ_chI2C_ADDRESS = 5108; // UE9 + U3
static const long LJ_chASYNCH_CONFIG = 5116; // UE9 + U3
static const long LJ_rgUNIP500V = 110; // 0V to +0.500V
static const long LJ_ioENABLE_POS_PULLDOWN = 2018; // U6
static const long LJ_ioENABLE_NEG_PULLDOWN = 2019; // U6
static const long LJ_rgAUTO = 0;
static const long LJ_chSWDT_UDPATE_DIOA = 5205;
